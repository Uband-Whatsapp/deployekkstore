                    ...
                    <div class="deploy-success">
                        <div class="deploy-success-header">
                            ...
                            <div class="deploy-success-info">
                                ...
                                <div class="deploy-success-subtitle">Sedang mempublikasikan ke Vercel...</div>
                            </div>
                        </div>
...
function showDeploySuccessState() {
    ...
    if (deployBtn) {
        ...
        deployBtn.innerHTML = '<span class="gate-spinner" style="width:14px;height:14px;border-color:rgba(255,255,255,0.3);border-top-color:#fff;display:inline-block;margin-right:6px;"></span><span id="deploy-btn-text">Sedang Mempublikasikan...</span>';
        deployBtn.dataset.state = 'success';
    }

    const urlEl = document.getElementById('deploy-url');
    if (urlEl) {
        const realUrl = urlEl.href || urlEl.textContent.trim();
        if (realUrl) pollDeployUrl(realUrl, 0);
    }
}

function pollDeployUrl(url, attempt) {
    const MAX_ATTEMPTS = 20;
    if (attempt >= MAX_ATTEMPTS) {
        finishPolling(false, url);
        return;
    }
    ...
    const timer = setTimeout(() => {
        if (finished) return;
        finished = true;
        setTimeout(() => pollDeployUrl(url, attempt + 1), 3000);
    }, 4000);
...
function finishPolling(isLive, url) {
    const deployBtn = document.getElementById('deploy-btn');
    const successSubtitle = document.querySelector('.deploy-success-subtitle');

    if (isLive) {
        if (successSubtitle) successSubtitle.textContent = 'Website sudah live dan siap diakses.';
        if (deployBtn) {
            deployBtn.disabled = false;
            deployBtn.classList.remove('btn-primary', 'btn-outline');
            deployBtn.classList.add('deploy-btn-new');
        ...
        showToast('Website sudah live!', 'success');
    } else {
        if (successSubtitle) successSubtitle.textContent = 'Vercel masih build. Coba buka manual setelah 1 menit.';
        if (deployBtn) {
            deployBtn.disabled = false;
            deployBtn.classList.remove('btn-primary', 'btn-outline');
            deployBtn.classList.add('deploy-btn-new');
        ...
        showToast('Vercel masih build, tunggu 1 menit', 'warning');
    }
}
...
async function deploy() {
    ...
    if (!projectInputEl || !fileInputEl) return;
